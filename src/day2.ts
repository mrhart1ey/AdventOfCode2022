...
const readInput = (): Game[] => {
    ...
    return lines.map((line) => {
        ...
        return {opponentsMove: inputOpponentMoveSymbolToMove(opponentsMove), result: inputGameResult(gameResult)};
    });
};
...
const calculateScoresForEachGame = (games: Game[]) => games.map((game) => scoreForGameResult(game.result) + scoreForMyMove(game))
